    ...
    class RevDiffMutator(irmutator.IRMutator):
        ...
        def mutate_ifelse(self, node):
            ...
            return loma_ir.IfElse(\
                node.cond,
                irmutator.flatten([self.mutate_stmt(stmt) for stmt in reversed(node.then_stmts)]),
                irmutator.flatten([self.mutate_stmt(stmt) for stmt in reversed(node.else_stmts)]),
                lineno = node.lineno)
