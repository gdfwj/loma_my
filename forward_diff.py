...
def forward_diff(diff_func_id : str,
    ...
    class FwdDiffMutator(irmutator.IRMutator):
        ...
        def mutate_ifelse(self, node):
            # HW3: TODO
            return loma_ir.IfElse(self.mutate_expr(node.cond),
                [self.mutate_stmt(stmt) for stmt in node.then_stmts],
                [self.mutate_stmt(stmt) for stmt in node.else_stmts],
                lineno = node.lineno)
        ...
        def mutate_less(self, node):
            return loma_ir.BinaryOp(\
                loma_ir.Less(),
                self.mutate_expr(node.left)[0],
                self.mutate_expr(node.right)[0],
                lineno = node.lineno,
                t = node.t)
